		...
		function tourmaster_ajax_send_enquiry_form(){
			...
			if( $recaptcha == 'enable' && $recaptcha_result !== true ){
				...
				if( !empty($data['email-address']) && is_email($data['email-address']) ){
					...
					$admin_mail_content = get_post_meta($data['tour-id'], 'enquiry-form-mail-content-admin', true);
					...
					$admin_mail_content = get_post_meta($data['tour-id'], 'enquiry-form-mail-content-customer', true);
